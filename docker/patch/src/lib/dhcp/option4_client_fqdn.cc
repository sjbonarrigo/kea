...
Option4ClientFqdnImpl::parseWireData(OptionBufferConstIter first,
    ...
    //*****************************************************************
    // Workaround to avoid droping Grapes packets with N=1 and S=1
    if((flags_ & Option4ClientFqdn::FLAG_N) != 0){
        if((flags_ & Option4ClientFqdn::FLAG_S) != 0){
            flags_ ^= Option4ClientFqdn::FLAG_S;
        }
    }
    //*****************************************************************
